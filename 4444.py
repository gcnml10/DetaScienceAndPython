...
class related_groceries:
    ...
    def find_related_groceries(self, grocery ,common_number=3):
        ...
        for month in range(1, 13):
            month_groceries = []
            month_lines = self.get_lines_with_keyword(grocery, month)  #month와 keyword=grocery 넣고 특정 달에서 keyword가 있는 라인 추출
    ...
    def all_frequency(self):
        ...
        f = open('mid-term_01.csv', 'w')
        f.write("grocery_name,01,02,03,04,05,06,07,08,09,10,11,12\n")
        for item in set_groceries:
            frequency = self.frequency(item)
            f.write(str(tuple(frequency.values())).strip('(').strip(')').strip("'").strip("'")) #tuple('grocery',,,) 형태이 데이터를 저장하기 위해 1,2,3 과 같은 형태로 변환
            f.write('\n')                                                                       #개행문자 삽입
        f.close()
    ...
    def Whole_Jaccard(self, threshold):
        ...
        #     print(frequency_set_01[item])
        #     for item_02 in set_of_groceries_02:
        #         if item_01 == item_02:
        #             continue                                #자기 자신이면 continue로 넘어가기
        #         else:
        #             Jaccard = self.Jaccard_formula(frequency_set_01[item], frequency_set_02[item])
        #             if Jaccard > threshold:                 #threshold 초과시 print
        #                 print(str(item_01) + ", " + str(item_02) + ", " + str(Jaccard))
        #   # del(set_of_groceries_02[item_01])             #중복계산을 피하기 위해 두번째 반복문의 set에서 삭제
